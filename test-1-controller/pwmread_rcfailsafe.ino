...
void pci_Setup(unsigned char pin){
    // enable pin
    *digital_Pin_To_PCMSK(pin) |= bit (digital_Pin_To_PCMSKbit(pin));
    // clear any outstanding interrupt
    PCIFR  |= bit (digital_Pin_To_PCICRbit(pin));
    // enable interrupt for the group
    PCICR  |= bit (digital_Pin_To_PCICRbit(pin));                   
}
...
void setup_pwm_read(){
  ...
  pwm_PIN_to_port();                             
  // pwm_PIN_to_port() also coverts the pin number in pwm_PIN[] (i.e. pin 11 or pin A0) to the pin position in the port register (i.e. 0b00000001) for use in the ISR.
